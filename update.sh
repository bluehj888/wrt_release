...
remove_unwanted_packages() {
    local luci_packages=(
        ...
        "luci-app-openclash" "luci-app-mihomo" "luci-app-appfilter" "luci-app-msd_lite" "luci-app-ddns"
    )
    local packages_net=(
...
install_small8() {
    ./scripts/feeds install -p small8 -f xray-core xray-plugin dns2tcp dns2socks haproxy hysteria \
        ...
        easytier luci-app-easytier msd_lite luci-app-msd_lite cups luci-app-cupsd luci-app-ddns
}
...
install_opkg_distfeeds() {
    # 检查是否使用 OPKG 包管理器
    if [ "$USE_APK" = "true" ]; then
        echo "使用 APK 包管理器，跳过 OPKG distfeeds 配置"
        return 0
    fi
    
    echo "配置 OPKG distfeeds..."
